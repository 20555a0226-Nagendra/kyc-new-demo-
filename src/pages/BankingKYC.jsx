...
const STORAGE_KEY = 'kyc-banking-saved-records'
...
const FORM_KEYS = [
  ...
  'occupation', 'accountType', 'annualIncome', 'relatedPersonName', 'relatedPersonRelation', 'relatedPersonAddress',
  'anyPolicy', 'passport', 'passportExpiryDate', 'remark',
]
...
const REFERENCE_COLUMNS = [
  ...
  { key: 'photoAttachment', label: 'PHOTO ATTACHMENT' },
  { key: 'residentialStatus', label: 'RESIDENTIAL STATUS' },
  { key: 'occupation', label: 'OCCUPATION' },
  { key: 'accountType', label: 'ACCOUNT TYPE' },
  { key: 'annualIncome', label: 'ANNUAL INCOME' },
  { key: 'relatedPersonName', label: 'RELATED PERSON NAME' },
  { key: 'relatedPersonRelation', label: 'RELATED PERSON RELATION' },
  { key: 'relatedPersonAddress', label: 'RELATED PERSON ADDRESS' },
  { key: 'anyPolicy', label: 'ANY POLICY' },
...
const accountTypes = ['Savings', 'Current', 'NRE', 'NRO', 'FD']
...
function generateRandomReference() {
  ...
  const expD = String(Math.floor(Math.random() * 28) + 1).padStart(2, '0')
  const expM = String(Math.floor(Math.random() * 12) + 1).padStart(2, '0')
  const expY = 2028 + Math.floor(Math.random() * 5)
  return {
    imageName: `BXDRCDIMG_AA${Math.floor(Math.random() * 90) + 10}_enc.jpg`,
    recordNo: String(Math.floor(Math.random() * 900000) + 100000),
    kycNumber: `BKI#${Math.floor(Math.random() * 9000000) + 1000000}`,
    name: `${f} ${l}`,
    guardianName: `${f2} ${l2}`,
    ...
    relatedPersonName: `${firstNames[Math.floor(Math.random() * firstNames.length)]} ${lastNames[Math.floor(Math.random() * lastNames.length)]}`,
    relatedPersonRelation: ['Spouse', 'Parent', 'Sibling', 'Guardian'][Math.floor(Math.random() * 4)],
    relatedPersonAddress: `${Math.floor(Math.random() * 99) + 1} Park Ave`,
    anyPolicy: ['Yes', 'No', 'N.A'][Math.floor(Math.random() * 3)],
    passport: `A${Math.floor(Math.random() * 9000000) + 1000000}`,
    passportExpiryDate: `${expD}-${expM}-${expY}`,
    remark: ['Nil', 'Verified', 'Pending'][Math.floor(Math.random() * 3)],
  }
}
                ...
                {savedRecords.map((record) => (
                  <div key={record.id} className="border border-slate-200 rounded-lg p-4 bg-slate-50/50 hover:bg-slate-50">
                    <div className="text-sm text-slate-700 space-y-1 mb-4">
                      ...
                      <p><span className="font-medium text-slate-500">Guardian:</span> {record.guardianName || '—'}</p>
                      <p><span className="font-medium text-slate-500">Account Type:</span> {record.accountType || '—'}</p>
                    </div>
